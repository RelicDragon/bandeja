...
export type KlikterenSnapshotResponse = SnapshotResponse;
...
async function assertKlikterenClub(clubId: string) {
  ...
  if (club.integrationType !== ClubIntegrationType.KLIKTEREN) {
    throw new ApiError(400, 'Club is not configured for Klikteren');
  }
  return club;
}
...
export async function getKlikterenSnapshot(
  clubId: string,
  date: string,
): Promise<KlikterenSnapshotResponse> {
  await assertKlikterenClub(clubId);
  parseDateParam(date);

  const rows = await prisma.clubKlikterenBusySnapshot.findMany({
    where: { clubId, date },
...
export async function replaceKlikterenSnapshot(
  userId: string,
  clubId: string,
  input: {
    date: string;
  ...
  const existing = await prisma.clubKlikterenBusySnapshot.findMany({
  ...
  const createRows: Prisma.ClubKlikterenBusySnapshotCreateManyInput[] = courts.map((court) => ({
    clubId,
  ...
  await prisma.$transaction(async (tx) => {
    ...
    await tx.clubKlikterenBusySnapshot.deleteMany({
      where: { clubId, date: input.date },
    });
    if (createRows.length > 0) {
      await tx.clubKlikterenBusySnapshot.createMany({ data: createRows });
    }
  });
