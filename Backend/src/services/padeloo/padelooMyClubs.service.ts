...
export async function getMyPadelooClubs(userId: string): Promise<PadelooMyClubsPayload> {
  const user = await prisma.user.findUnique({
    where: { id: userId },
    select: { currentCityId: true },
  });

  const auths = await prisma.userClubPadelooAuth.findMany({
    where: { userId },
  ...
  const clubs: PadelooMyClubRow[] = allClubs.map((club) => {
    ...
    return {
      ...
      clubName: club.name,
      avatar: club.avatar,
      padelooClubId: config?.clubId ?? null,
      connected: !!auth,
  ...
  return {
    cityPadelooClubCount: cityClubs.length,
