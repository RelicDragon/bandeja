...
/** Padel-only activity arms for idle decay (per-sport profiles; grace counter stays on User). */
export const reliabilityDecayPadelActivityArms = Prisma.sql`
  outcome_times AS (
    SELECT go."userId" AS uid, MAX(go."createdAt") AS t
    FROM "GameOutcome" go
    INNER JOIN "Game" g ON g.id = go."gameId"
    INNER JOIN eligible e ON e.id = go."userId"
    WHERE g.sport = ${Sport.PADEL}::"Sport"
      AND g."entityType" NOT IN ('BAR'::"EntityType", 'LEAGUE_SEASON'::"EntityType")
    GROUP BY go."userId"
  ),
  game_times AS (
