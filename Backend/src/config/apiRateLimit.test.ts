...
function run() {
  ...
  const override = resolveApiRateLimitConfig({
    ...
    maxEnv: '500',
    windowMsEnv: '60000',
    skipPathPrefixesEnv: '/health,/custom',
  });
  ...
  assert.equal(shouldSkipApiRateLimit('/chat/messages/missed', skips), true);
  ...
  assert.equal(shouldSkipApiRateLimit('/users/me?redirect=/chat/unread', skips), false);
