/**
 * Append Ireland clubs from Padel Lands JSON (dedupe by cityId + normalizedName).
 *
 *   DB_URL=... DB_SCHEMA=padelpulse npx ts-node -r dotenv/config scripts/seed-ireland-padellands.ts
 *   DRY_RUN=1 ...
 */
...
const DATA_PATH = path.join(__dirname, 'data', 'ireland-padellands-clubs.json');
...
async function getOrCreateCity(
  ...
  try {
    ...
    console.warn(`[ireland-pl] cityGroup skip for ${name}:`, (e as Error)?.message || e);
  }
...
async function main(): Promise<void> {
  ...
  console.log(`[ireland-pl] loaded ${raw.length}, usable ${clubs.length}, dryRun=${DRY_RUN}`);
  ...
  console.log(`[ireland-pl] existing Ireland clubs ${existing.length}`);
  ...
  for (const pl of clubs) {
    ...
    if (clubsCreated % 50 === 0) {
      console.log(
        `[ireland-pl] progress created=${clubsCreated} citiesCreated=${citiesCreated} skippedDupe=${skippedDupe}`
      );
    }
  }
  ...
  console.log(
    `[ireland-pl] done created=${clubsCreated} citiesCreated=${citiesCreated} skippedDupe=${skippedDupe} skippedBad=${skippedBad} citiesRefreshed=${citiesRefreshed}`
  );
}
