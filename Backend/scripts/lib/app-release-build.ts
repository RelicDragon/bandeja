...
export function runBuildPreflight(platform: ReleasePlatform = 'both'): BuildPreflight {
  const issues: string[] = [];
  const needsAndroid = includesAndroid(platform);
  const needsIos = includesIos(platform);

  if (!fs.existsSync(path.join(FRONTEND_DIR, 'package.json'))) {
    issues.push(`Frontend directory not found at ${FRONTEND_DIR}`);
  }

  if (needsAndroid && !fs.existsSync(path.join(ANDROID_DIR, 'gradlew'))) {
    issues.push('Android Gradle wrapper missing — run Capacitor sync first.');
  }

  const releaseJavaHome = needsAndroid ? resolveJavaHome() : undefined;
  if (needsAndroid && !releaseJavaHome) {
  ...
  if (needsAndroid && !process.env.ANDROID_HOME && !process.env.ANDROID_SDK_ROOT) {
    issues.push('Set ANDROID_HOME or ANDROID_SDK_ROOT for Android release builds.');
  }

  if (needsAndroid && !androidSigningConfigured()) {
    issues.push(
...
export async function runReleaseBuild(
  ...
  const releaseTasks = [
    ...
    timedListrTask(timer, `Capacitor sync (${capSyncTarget})`, async () => {
      ...
      await runCommand('npx', capSyncArgs, {
        cwd: FRONTEND_DIR,
        env: sharedEnv,
      });
      if (needsIos) {
  ...
  const tasks = new Listr(releaseTasks, { concurrent: false, exitOnError: true });
