/**
 * Append Latvia clubs from Padel Lands JSON (dedupe by cityId + normalizedName).
 *
 *   DB_URL=... DB_SCHEMA=padelpulse npx ts-node -r dotenv/config scripts/seed-latvia-padellands.ts
 *   DRY_RUN=1 ...
 */
...
const DATA_PATH = path.join(__dirname, 'data', 'latvia-padellands-clubs.json');
...
async function main(): Promise<void> {
  ...
  console.log(`[latvia-pl] loaded ${raw.length}, usable ${clubs.length}, dryRun=${DRY_RUN}`);
  ...
  console.log(`[latvia-pl] existing ${COUNTRY} clubs ${existing.length}`);
  ...
  for (const pl of clubs) {
    ...
    if (clubsCreated % 50 === 0) {
      console.log(
        `[latvia-pl] progress created=${clubsCreated} citiesCreated=${citiesCreated} skippedDupe=${skippedDupe}`
      );
    }
  }
  ...
  if (!DRY_RUN && !SKIP_CITY_REFRESH) {
    ...
    console.log('[latvia-pl] skip refreshAllCitiesFromClubs (SKIP_CITY_REFRESH)');
  }

  console.log(
    `[latvia-pl] done created=${clubsCreated} citiesCreated=${citiesCreated} skippedDupe=${skippedDupe} skippedBad=${skippedBad} citiesRefreshed=${citiesRefreshed}`
  );
}
