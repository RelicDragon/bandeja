/**
 * Append Lithuania clubs from Padel Lands JSON (dedupe by cityId + normalizedName).
 *
 *   DB_URL=... DB_SCHEMA=padelpulse npx ts-node -r dotenv/config scripts/seed-lithuania-padellands.ts
 *   DRY_RUN=1 ...
 */
...
const DATA_PATH = path.join(__dirname, 'data', 'lithuania-padellands-clubs.json');
...
const SKIP_CITY_REFRESH = process.env.SKIP_CITY_REFRESH !== '0';
...
async function main(): Promise<void> {
  ...
  console.log(`[lithuania-pl] loaded ${raw.length}, usable ${clubs.length}, dryRun=${DRY_RUN}`);
  ...
  console.log(`[lithuania-pl] existing ${COUNTRY} clubs ${existing.length}`);
  ...
  for (const pl of clubs) {
    ...
    if (clubsCreated % 50 === 0) {
      console.log(
        `[lithuania-pl] progress created=${clubsCreated} citiesCreated=${citiesCreated} skippedDupe=${skippedDupe}`
      );
    }
  }
  ...
  if (!DRY_RUN && !SKIP_CITY_REFRESH) {
    ...
    console.log('[lithuania-pl] skip refreshAllCitiesFromClubs (SKIP_CITY_REFRESH)');
  }

  console.log(
    `[lithuania-pl] done created=${clubsCreated} citiesCreated=${citiesCreated} skippedDupe=${skippedDupe} skippedBad=${skippedBad} citiesRefreshed=${citiesRefreshed}`
  );
}
