...
`GameResultsEntryEmbedded` when `resultsStatus !== NONE` (not BAR/TRAINING/season hub). Start on ANNOUNCED requires confirm. Reset all / finish / edit FINAL = danger confirms. Finish sits in a sticky bar with match progress; its confirmation summarizes unscored / incomplete matches and ties. **Restart is always visible in that sticky bar while results are IN_PROGRESS** (beside the progress line; alone and full-width before any match is ready to finish) so a scorer can always cancel entry and return to game setup — never hide it behind a menu. It is gated on edit *permission* only (`canUserEditGameFormat`, the mirror of the server's `canModifyResults` reset guard), not on `canEdit`: when the roster stops being ready mid-entry (`participantsReady` false, fixed teams broken) the board turns read-only and Restart is the only way back to setup. Edit FINAL lives in the ⋯ beside the results tabs.

Board interaction (`ResultsRoundsBoard`, helpers in `utils/resultsBoardNavigation.ts`): lineups are edited by tapping seats — an empty seat focuses the match and the player tray places into the highlighted side; a placed player opens move / swap / remove. Move, swap (also across matches of one round) and auto-fill are one `GameResultsEngine.setMatchLineups` edit, persisted as one match PUT per changed match. A full match shows **Enter score**; **Save and next** in the score dialog advances to the next set/match. When the next row is in the *same* match it waits for this save to land and opens a fresh draft on the resulting version — it never carries the old draft's `baseVersion` forward. When the draft score ends the match (e.g. 6–4 6–4 in best of 3; `matchOutcomeAfterSet`), the dialog shows who won (or a draw) above the footer and hides **Save and next** unless another match is ready to score. Automatic (flexible) scoring never counts as ended there, since more sets may follow a win.
