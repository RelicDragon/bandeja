...
Watch merge rule (`MatchScoringViewModel`): a newer remote revision that lands while local taps are unacknowledged is **applied**, then the unacknowledged taps are replayed on top and pushed on the new base. The watch never adopts a revision without its state (that would let the next PATCH erase the other writer's points). A non-409 rejection (400 invalid sets / out of graph, 403) drops the local edits and re-syncs to the server envelope, like the web `refresh`. The watch mirrors server auto-advance: completed rally games and the empty optional-decider row are stepped onto before the PATCH so the state stays inside the transition graph.
