...
- **Organizer next steps (PRD 364)**: `Frontend/src/features/organizer-next-actions/`. One `Card` under the header, for owner/admin (`canEdit`) and, seats-only, for a participant with invite rights. `buildOrganizerNextActions` is a pure `(input) → Hint[]` over data the page already has: seats (PLAYING vs `maxParticipants`, queue length) → booking gap (`resolveGameBookingBadgeKind`, read from the payload, never a provider) → attendance (`useGameAttendance` details) → cost (the Cost card's own query, `UNPAID` non-payer shares). Before results all four apply; once `resultsStatus !== NONE` only cost. Two rows visible, the rest behind "+N more". Zero hints → nothing rendered. Each action lands on the existing workflow (invite picker, `#game-join-queue`, `#game-linked-bookings`, `EditGameInfoModal` location tab, the Nudge mutation, `[data-cost-card]`, `?settle=1`). On by default; the platform setting `GAME_ORGANIZER_NEXT_ACTIONS_ENABLED` (read via `GET /public/platform-flags`, `usePlatformFlags`, default in `PUBLIC_PLATFORM_FLAG_DEFAULTS`) is the rollback switch — `false` turns it off. With the block on, `resolveOrganizerSurfacePlacement` moves the attendance organizer strip (`AttendanceCard showOrganizerStrip={false}`) and the open-spot row (`GameQueuePanel hideOpenSpotRow`) into the block so neither is on screen twice. Flag off is byte-for-byte the previous page. Not for LEAGUE, LEAGUE_SEASON, EVENT or ARCHIVED.
...
`GameSettings` toggles: `affectsRating`, `isPublic`, `anyoneCanInvite`, `suitableForNovices`, `resultsByAnyone`, `allowDirectJoin`, `autoFillFromQueue`, `afterGameGoToBar`, `showOnLiveRail`.

- **`autoFillFromQueue`** sits immediately below `allowDirectJoin`, with a read-only "N in queue" line under it.
- **`showOnLiveRail`** (`Boolean @default(true)`) is the organizer's opt-out from the Live now rail, independent of `isPublic`: a private game is never on the rail whatever this says (except a league fixture of a public season — fixtures are always created private), and a public game with it off stays joinable but is invisible on the rail, live or finished. It is editable through the ordinary `PUT /games/:id` path (it is in `GAME_UNCHECKED_SCALAR_KEYS`) and is rendered only while the game could reach the rail (`isRailVisibleGame`: public, or a public season's fixture). `readSetting` in `GameSettings.tsx` defaults every other toggle to `false`; this one must default to **`true`** (`game.showOnLiveRail ?? true`), or every game created before the column existed reads as hidden until its owner touches the switch. See [live-scoring.md](./live-scoring.md).
- **`suitableForNovices`** (PRD 360, `Boolean @default(false)`) sits immediately below `anyoneCanInvite`. It is the organizer's "Novices welcome" promise and changes **no** gate: level range, gender rule, direct-join and queue rules are untouched. Offered only where `entityCapabilities.hasNoviceTag` is true (GAME, TOURNAMENT, TRAINING, BAR) — create and update both force/drop it otherwise — and it is in `GAME_RESULTS_LOCKED_FIELDS`, so it freezes with the other settings once results entry starts. Display and the Find filter: [home-and-find.md](./home-and-find.md).
...
- Duplicate: `canEdit` + `resultsStatus === 'NONE'` → `/create-game` with `initialGameData` (`buildDuplicateGameInitialData`, copies the old time/court/booked flag; invites nobody). Hidden during `IN_PROGRESS` and after `FINAL`.
- **Play with this group again** (PRD 362): after `FINAL`, the primary action of the results area (`PlayWithGroupAgainButton`; in `GameResultsShareCard` for GAME/TOURNAMENT, mounted directly in the TRAINING/BAR results blocks). Visible to PLAYING participants and to the TRAINING `trainerId`; gated on `entityCapabilities.canRematch` (GAME, TOURNAMENT, TRAINING, BAR). Opens `/create-game` with `buildRematchGameInitialData` (format allow-list; `REMATCH_EXCLUDED_KEYS` strips court, schedule, booking, series, trainer, cost, results) and the previous PLAYING roster as `invitedPlayerIds` + `invitedPlayers` (`previousRosterInvitees`, viewer excluded). TRAINING: the previous trainer is re-invited with `asTrainer` unless the creator is the trainer (`creatorNonPlaying`). Nothing is sent until the game is created; nobody is seated.
