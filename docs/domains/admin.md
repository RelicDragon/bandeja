...
| Platform Settings | `platformSettingsPage` | Results-artifact Replicate photo model (`GET/PATCH /admin/results-artifacts/photo-model`); **Cost Split** `COINS_PER_CURRENCY_UNIT` (deliberately unset — while null the coins settle option is hidden on both ends); `REFERRAL_REWARD_REFERRER` / `REFERRAL_REWARD_REFERRED`; **Organizer Next Steps** `GAME_ORGANIZER_NEXT_ACTIONS_ENABLED` (PRD 364 rollback switch: **on by default**, no row = on, `false` turns it off; published read-only with **Find: Looking-to-play Count** `FIND_LOOKING_COUNT_ENABLED` (PRD 363, same On/Off card; **on by default**, off only when the row is literally `false`) through the unauthenticated `GET /api/public/platform-flags`, allow-listed in `Backend/src/services/platformFlags.service.ts`) |
