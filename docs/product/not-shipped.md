...
| Attendance actions in the push shade | PRD 346 emits both action tokens and sets `nativeHandler: 'attendance_actions'`, and Telegram's inline buttons work. The Android branch (`InviteNotificationHelper`/`InviteActionReceiver` equivalents) and the iOS `UNNotificationCategory` registration are not written, so "I'm coming" from the lock screen is Telegram-only. |
