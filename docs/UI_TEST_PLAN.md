...
| PR-trophy-2c | Habit family stack | Own Statistics with ≥2 unlocked `habit_games_*` (or wins/streak) | Same-family unlocks show as one piled stack (best on top / leftmost when expanded); locked same-family is a separate pile; single unlock stays a normal card; carousel leftmost is the better trophy |
