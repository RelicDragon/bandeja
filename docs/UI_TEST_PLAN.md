...
| GD-123 | Equal-wins mini-tables on standings | Fixed-team or 1v1; ≥2 tied on wins (not 0–0–0); Standings | Tied rows highlighted + **Why?** scrolls to mini block; section explains H2H vs mini; “Why above next” + highlighted deciding metric; order matches main table |
...
| PR-trophy-2b | Achievements carousel | Player card (after message/favorite) / Profile Statistics (after avatar, before followers) with several trophies | Single-row horizontal carousel (unlocked first); partial next tile peeks; single cards open detail; family stacks expand first |
| PR-trophy-2c | Habit family stack | Own Statistics with ≥2 unlocked `habit_games_*` / streak / wins (incl. `habit_first_win` + `habit_wins_*`) | Same-family unlocks show as one piled stack (best on top / leftmost when expanded); first win stacks with wins ladder; locked same-family is a separate pile; single unlock stays a normal card; carousel leftmost is the better trophy |
