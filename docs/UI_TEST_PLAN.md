...
| PR-84 | Other player preferred hand | Open `/user-profile/:id` or `?player=` for a user with only left hand set | L chip selected (filled); R unset (dashed outline), not both-on |
| PR-85 | Other player court side | Same view for a user with only left court side set | L chip selected; R unset |
| PR-86 | Unset vs selected preference chips | Open another player with neither hand nor court side set | All four chips look unset (outline), visually distinct from selected fill |
