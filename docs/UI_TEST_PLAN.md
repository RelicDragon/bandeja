...
| CH-13d | Stalled sync backs off | Force a thread whose pulls never advance the cursor (or return 429), then trigger repeated gap pulls via socket/foreground signals | Requests stop after a few attempts and resume ~30s later; no sustained request storm and no client-wide rate limiting |
