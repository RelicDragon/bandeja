...
| C-10c | Automatic set entry | Automatic game → set 1: SegmentedSwitch Set/games vs Americano points (match-level); set 2+ uses same mode; at 1–1 decider can pick super tiebreak; next match can differ | Set/games 0–10; after 1–1 third set defaults to match mode (not STB); STB only after switch; STB uses tiebreak point cap (first-to+5); hint/examples follow active mode; title shows STB suffix only when STB selected |
