...
| H-38j | Booking slot occupancy pill | Upcoming booking with partial or full linked game coverage | Small % pill beside slot time; grouped adjacent card shows weighted % across all slots; 2h booking linked to 1h game shows ~50% (not full) |
...
| F-66 | Find calendar keeps archived history | Busy city month with hundreds of ARCHIVED on early days and live games late | Early-day badges still reflect archived city games; selecting today/late day still lists live games via day-scoped fetch (not blank) |
| F-67 | Find empty day-scoped falls back to month slice | Select a day that exists in month cards; force day-scoped response `[]` with `hasMore=false` | List still shows month day-slice (not permanent “No games found”). If day-scoped `hasMore=true` and empty, keep day authority + Load more |
...
| F-69 | Find calendar type pills under truncate | Busy city month: early days in first 300 cards, late days only in dayIndex | Late-day cells still show entity-type pills (GAME/TRAINING/…), not badge-only empty cells |
