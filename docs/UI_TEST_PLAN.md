...
| PR-trophy-2c | Habit family stack | Own Statistics with ≥2 `habit_games_*` / streak / wins (incl. `habit_first_win` + `habit_wins_*`), some locked | Same-family locked + unlocked show as one piled stack (unlocked best→worst then next locked→max); green progress for intermediate chase, golden highlight + bar for max level; first win stacks with wins ladder; single entry stays a normal card; carousel leftmost is the better family |
