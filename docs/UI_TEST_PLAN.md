...
| GD-03a | Details stay mounted across layout changes | `@desktop` switch tournament table view on/off; reveal Event side chat after gaining access | Details retain local state and scroll position; no bootstrap refetch or repeated loading/entrance animation |
| GD-03b | Quiet details updates | Open game details, receive updates for other games/chat/navigation, then a name/roster update for this game | Unrelated store events do not rerender details; relevant data updates in place without clearing unchanged sport/format tags. Initial page data renders without a second skeleton or duplicate game fetch. Regression: `npm --prefix Frontend run test:game-details-render` |
...
| GD-213 | Substitute row appears during results entry | Owner starts results entry (`resultsStatus=IN_PROGRESS`) on a non-BAR game | A single compact "Substitute player" row with a one-line hint and a chevron is shown; no player list is rendered on the page. Tapping it opens the substitute modal with a fade-in, and closing it animates out |
| GD-214 | Card hidden when not applicable | View the same game as a non-owner, or with `resultsStatus` `NONE` / `FINAL`, or on a BAR game | No "Substitute player" row |
| GD-215 | Substitute an injured player | During results entry tap "Substitute player" → pick who leaves the court → pick a replacement who is not on the roster → confirm | Three-step modal: the first step lists every PLAYING participant except the trainer; the second shows "Replacing: <name>" with a Change link back to step one; the confirm step shows out/in names and the three consequences. On confirm a toast appears, the substitute is PLAYING and the outgoing player is no longer on court. Reopening the modal starts again at step one |
...
| GD-220 | Substitution refused for league games | Open a LEAGUE fixture (or league season) with `resultsStatus=IN_PROGRESS` as owner; also call the endpoint directly | No "Substitute player" row is shown, and the endpoint returns the league-not-supported error; league standings for the fixture are unchanged |
...
| GD-109 | Score entry modal layouts | Open set score modal in portrait and landscape | Portrait: 3-column grid — team avatars + first names top row, score tile with `− +` pills below and a two-dot colon centered on the tiles; landscape: two stacked team rows (avatars + names left, horizontal `− score +` right); no overlapping elements; leading score tile turns green (cross-fades when the lead changes hands). Header shows a context pill "Round N · Match M" (+ "Set K" for multi-set matches) above the title; the court chip sits on the examples line so the title keeps one line in every locale |
| GD-109a | Score entry visuals across themes | Open the modal in light, dark, Premium light and Premium dark | Scoreboard and keypad sit in soft trays with no hard borders; the tile being edited has a primary-colored halo that glides to the other tile when the keypad switches team; accents follow the Premium gold palette with no extra overrides |
...
| GD-110a | Score keypad selection never cropped | Open the keypad with the score at 0 (top-left key), then at the last key; repeat with a 0–30 points/timed range and in Premium dark | The selected key's fill, press pop and shadow render fully inside the keypad (no clipped edges at the scroll box or the team-slide viewport). Rows stay balanced: 0–7 → 4×2, 0–10 → 6+5, 0–30 + `…` fits without an inner scroll on a 375pt phone |
| GD-110b | Score entry footer labels | `@mobile` 375pt wide, Russian/Spanish, match with Save and next | "Save and next" is never truncated mid-word: long translations wrap to two balanced lines inside the pill; Save stays one line |
...
| GD-226a | Save and next when the score ends the match | Single-match Bo3 → set 1 6:4 → Save and next → set 2 enter 6:4 | A small "<names> won the match" hint appears above the footer and Save and next disappears (only Cancel + Save). Changing set 2 to 4:6 brings Save and next back and removes the hint; a third set 6:3/3:6 hides it again with the right winner. With another unscored match in the game, the hint shows but Save and next stays and opens that match. Automatic (flexible) scoring: no hint, Save and next unchanged |
...
| GD-232 | Sticky finish bar | Results in progress, teams ready | Finish sits in a bar pinned to the bottom while the board is on screen, with "X of Y matches scored" and a progress bar. Finish confirmation lists matches without a score / missing players (Go to jumps and pulses the card) and ties. A compact red **Restart** sits at the right of the progress line (Finish keeps the full width; fits in ru/id at 375 px; mirrors in `ar`); Restart → danger confirm → results reset and the game returns to setup. Edit results (after finishing) lives in the ⋯ next to the Results/Stats/Scores switch; round delete in the round header ⋯ |
| GD-232a | Restart always present while scoring | Start results, then: (1) before any match has both teams, (2) on the Stats tab, (3) mid-score with matches ready, (4) a player leaves so the roster is no longer full (board goes read-only) | Sticky bottom bar is on screen in every case: (1) and (4) show only a full-width "Restart Game/Tournament" button (no progress, no Finish); (2)/(3) show Restart on the progress line above Finish. Disabled while finishing/restarting. Players without results permission see no bar. Not in the ⋯ menu |
...
| GD-64a | Start results loading overlay | Tap "Start Results Entry" on a STARTED game, and separately on an ANNOUNCED game → Yes, Continue | A blurred backdrop with a spinner card ("Starting results entry…") appears immediately on tap / confirm and blocks the page; it fades out once the results entry UI is ready. On API failure the overlay clears and an error toast shows. Repeated taps don't start twice |
...
| GD-66c | Concurrent manual sessions | Two devices open the same match version and save different scores simultaneously | One commits; the other sees a conflict and the authoritative score. A fresh correction after reload can replace it; old retries cannot |
| GD-66d | Old offline board | A edits offline; B saves newer scores or adds a round; A reconnects and syncs | 409; B's scores/rounds survive; A must load the latest board before making a fresh correction |
| GD-66e | Rapid local edits | Delay the first save; enter a second score; then release replies | Writes retain entry order and use each preceding acknowledgement; last entered score wins within the session |
| GD-66f | Manual/live interference | Keep live board open on B; save a manual correction on A while B's request is delayed | B's old request conflicts; board loads A's corrected sets; delayed HTTP/socket replies cannot undo the correction; next fresh live action uses the retained revision |
| GD-66g | Finish versus stale client | B finishes while A has an old score request in transit | A cannot reopen or mutate FINAL results; Finish never uploads a cached board over B's newer scores |
| GD-66h | Watch offline conflict | Queue a Watch score; save a newer manual score on phone; reconnect Watch | Pending old score cannot replace the phone score; Watch loads corrected sets and requires a fresh scoring action |
| GD-66i | Restart during save | Close the app while a score request is pending, then reopen | The latest local draft remains recoverable with its original version; it cannot silently replace a newer saved score |
...
| GD-LN-06 | Hidden for private games | Turn **Public game** off | The **Show on Live now** row disappears — a private game can never be on the rail. Exception: a fixture of a public league season keeps the row (it can reach the rail through its season) |
...
| LN-02 | Rail appears | Start live scoring on a public game in the viewer's city, reload Find | Rail **above the calendar** with a red `● LIVE` dot and the label "Live now"; with two or more games a red count follows the label (no count for a single game) |
...
| LN-06 | Carousel | Two or more live games | Horizontal snap carousel of 264 px cards that bleeds to the rail's edges; the next card peeks in; swiping snaps to card boundaries |
| LN-07 | Single game | Exactly one live game | One full-width card instead of a carousel. Tapping anywhere on the card opens the watch board |
...
| ID | Test | Steps | Expected |
|----|------|-------|----------|
| LN-20 | Card content | Look at a card | A compact scoreboard, same layout as the watch board: club tile + club · court on top; then one row per side (plain faces with no level badges, names, a column per set, the point score in a highlighted chip on the end); footer "Started 23 min ago" ("Started 11 h ago" past the first hour) on the start side and a **Watch** pill on the end. The whole card is one tap target; tapping a face does **not** open a player card |
| LN-21 | Rating icon | Rated game vs unrated | Small rating icon in the header only for the rated one |
| LN-22 | Score motion | Score a point on the live board in another session | The changed digit slides vertically (200 ms); unchanged digits do not move. The leading side's names are bold, the trailing side's muted. `@two-user` |
| LN-23 | Reduced motion | Repeat LN-22 with reduced motion | Digit changes instantly, new value correct. `@manual` |
| LN-24 | Long names | Card with long player names, three sets | Names truncate with an ellipsis; set and point columns stay aligned between the two rows and never shrink |
| LN-25 | Sets and points | First set in progress; then a later set; then a `points`-mode sport | One set column plus the point chip; completed sets show the loser's games dimmed and the running set on a tinted cell; a sport with no sub-game score has no point chip |
...
| ID | Test | Steps | Expected |
|----|------|-------|----------|
| LN-30 | Skeletons | Throttle the network and open Find | Exactly two score-card skeletons shaped like the scoreboard (venue, two rows, footer); the header dot is already live |
| LN-31 | Socket loss | Kill the socket (airplane mode, or stop the backend) with the rail on screen | Each card keeps its last score (slightly dimmed) and the footer switches to a small grey wifi-off "Reconnecting". Nothing blanks out, no card disappears. `@manual` |
...
| LN-39 | Not on the rail | Americano / round-robin (several matches); walkover or technical result; results reopened for editing | No finished card: multi-match formats have no single scoreline, walkovers never stamp a finish time, reopening clears it |
...
| ID | Test | Steps | Expected |
|----|------|-------|----------|
| LN-40 | Non-participant watch | Tap a card as a non-participant | `/games/:id/watch` opens the spectator stage for padel / tennis: the court from above — team A's faces and names at the far end, team B's at the near end (lime ball badge on the server's face), each side's numbers in its service box facing the net: completed sets small (the set's loser dimmed), the running set's games large, the game points on a tile at the end. A header reads **Set N** (+ the match timer when on). Rally sports keep the TV rally board. No "not allowed" error, no login wall, no scoring buttons, no scorer toolbar or QR on tap |
| LN-41 | Entry motion | Open the board; then with reduced motion; then watch a few points | While loading, a stage-shaped skeleton holds the space (no jump when the score arrives). The stage fades up out of a soft blur (~0.9 s) and the two halves settle one after the other; each new number pops in, the ahead-in-the-game side's tile turns the primary colour and its side of the net lights up softly. Under reduced motion everything simply appears. `@manual` |
| LN-41b | Sizes | `@mobile` portrait phone; landscape phone; tablet; scorer's TV (`/live?tv=1`) | Portrait phone: the stage fills the height under the strip. Landscape phone: the page scrolls rather than squashing the stage. Tablet/desktop: strip and stage share one centred column. Long names truncate instead of pushing the numbers; set and point columns line up across the net; follows the app's light/dark appearance. `?tv=1` is the scorer's TV board and is unchanged (TV: `?theme=`) |
| LN-41c | Point states | Deuce; advantage; golden point (40–40 with golden point on); tie-break at 6–6; americano (points mode) | A chip on the net: **Deuce** / **Advantage** (tile shows **AD**) / **Golden point** (gold) / **Tie-break**; in a tie-break the tile caption reads **TB** and shows tie-break points. Americano: the running total sits on the tile, captioned **Points**. Match decided: tiles and serve ball gone, final set scores large, the winners carry a **Winner** badge, the losers dimmed, header reads **Match complete** (**Match drawn** on a draw) |
...
| LN-43 | Spectator strip | Look at the strip | A floating glass strip: Back, `● LIVE │ Padel Centar · court 3`, and a **Follow players** button (person-plus). Offline: an **Offline** pill floats under it without moving the board. Back returns to the game page, not out of the app |
...
| LN-44 | Follow from overflow | Open **Follow players** → tap a player | The menu scales in from the button; every player from both sides is listed with their face; tapping one gives a success toast, turns the row's **+** into a checkmark and adds the player to the viewer's following list |
...
| LN-47 | Rail privacy | Private casual game; league fixture of a **private** season; game with **Show on Live now** off | None of them ever appear on the rail (live or finished), and the spectator endpoint answers a plain 404 so it cannot be used to probe whether a private game exists. A FINAL game never gets a spectator token |
| LN-48 | League fixtures | Live-score a fixture of a **public** league season (fixtures are created private) | It appears on the rail with an amber outline and a ribbon "🏆 League name · Round N" (**Playoff** for bracket games); league fixtures sort ahead of casual games in the same phase. Tapping it opens the watch board for a stranger |
