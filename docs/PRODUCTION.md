...
Interactive flow: choose target (**Both** by default, **Android**, or **iOS**), read the latest uploaded version/build from Google Play and/or App Store Connect (override manually if needed), draft What's new (AI, custom, or template), build the selected signed artifact(s), upload to the selected store(s), verify store state, then update the What's-new baseline commit. Non-interactive target: `./scripts/app-release.sh --platform android|ios|both`. iOS destinations: **TestFlight Internal** (default), **TestFlight Beta** (external groups via prompt or `TESTFLIGHT_GROUPS`), prepare-without-submit, or submit-for-review. TestFlight uploads do not attach a build to an App Store version, do not cancel an in-progress App Review, and do not move the shipped baseline on iOS-only runs. Before a production/review upload, the live planner detects an existing Google Play or App Store review and requires explicit approval to replace/remove it; a two-store conflict can be approved for both or narrowed to one platform. Apple removal finishes before IPA upload, while Google replacement is atomic and bound to the approved production artifact set. The stores are rechecked before upload, and Apple is guarded again at final review submission. The iOS App Store upload waits for App Store Connect processing before it writes What's New metadata and optionally submits for review; if Apple is still processing, resume later with `APP_RELEASE_RESUME=1`. Planner-only rehearsal: `APP_RELEASE_DRY_RUN=1` (still reads live store versions unless overridden via env).
...
Native builds are cross-origin (`https://localhost` → `bandeja.me`) and web is same-origin. A request header the client sends but `CORS_ALLOWED_HEADERS` (`Backend/src/config/corsHeaders.ts`) omits is dropped by the browser **before it is sent**: the app shows a generic connection error and the server logs nothing. Web keeps working, so the breakage looks app-only. Adding a header in `Frontend/src/api/` means adding it there too; `corsHeaders.test.ts` scans the frontend and fails on drift.
