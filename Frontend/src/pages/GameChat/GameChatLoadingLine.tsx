...
export function GameChatLoadingLine({ show }: { show: boolean }) {
  ...
  if (reduceMotion) {
    return show ? (
      <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-primary-400/50" />
    ) : null;
  }
  ...
  return (
    <AnimatePresence initial={false}>
      {show && (
        <motion.div
          className="absolute bottom-0 left-0 right-0 h-0.5 bg-gradient-to-r from-primary-400/0 via-primary-400/80 to-primary-400/0 dark:from-primary-500/0 dark:via-primary-500/80 dark:to-primary-500/0"
