...
function ClubListItemInner({ club, isSelected, isNearest, onSelect, scrollTargetRef, className = '' }: ClubListItemProps) {
  ...
  return (
    <div
      ref={scrollTargetRef}
      className={`w-full min-w-0 text-left overflow-hidden rounded-xl border transition-colors flex ${
        isSelected
    ...
    >
      <div className="relative w-12 shrink-0 self-stretch min-h-[4.5rem]">
        <ClubAvatar club={{ avatar: club.avatar ?? null, name: club.name, id: club.id }} variant="tile" />
      </div>
      <div className="flex-1 min-w-0 flex flex-col py-3 pr-4 pl-3">
        <button
          type="button"
