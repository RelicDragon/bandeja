...
export const rarityLabelKey = (rarity: TrophyRarity): string => {
  if (rarity === 'UNIQUE') return 'trophies.rarity.unique';
...
/** Rare+ get a tag; common stays unlabeled. */
...
export function rarityFrameClass(rarity: TrophyRarity, locked: boolean): string {
  ...
  if (rarity === 'UNIQUE') {
    return 'from-fuchsia-200 via-violet-400 to-amber-300 ring-fuchsia-400/55 dark:from-fuchsia-300 dark:via-violet-500 dark:to-amber-500 dark:ring-fuchsia-400/40';
  }
  if (rarity === 'LEGENDARY') {
...
export function rarityTextClass(rarity: TrophyRarity, locked: boolean): string {
  ...
  if (rarity === 'UNIQUE') return 'text-fuchsia-700 dark:text-fuchsia-300';
...
export function rarityBadgeClass(rarity: TrophyRarity, locked = false): string {
  ...
  if (rarity === 'UNIQUE') {
    return 'bg-fuchsia-100 text-fuchsia-950 ring-fuchsia-300/90 dark:bg-fuchsia-950/80 dark:text-fuchsia-100 dark:ring-fuchsia-400/50';
  }
  if (rarity === 'LEGENDARY') {
...
export function rarityGlowClass(rarity: TrophyRarity, locked: boolean): string {
  if (locked) return '';
  if (rarity === 'UNIQUE') return 'shadow-[0_0_26px_rgba(217,70,239,0.5)]';
...
export function rarityAuraClass(rarity: TrophyRarity): string {
  if (rarity === 'UNIQUE') {
    return 'from-fuchsia-400/55 via-violet-300/35 to-transparent';
  }
  if (rarity === 'LEGENDARY') {
...
export function rarityCelebrationShell(rarity: TrophyRarity): string {
  if (rarity === 'UNIQUE') {
    return 'border-fuchsia-300/60 from-fuchsia-50 via-white to-violet-50 dark:border-fuchsia-500/40 dark:from-fuchsia-950/85 dark:via-gray-950 dark:to-violet-950/55';
  }
  if (rarity === 'LEGENDARY') {
