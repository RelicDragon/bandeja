...
type NspadelCreateGameConfirmModalProps = {
  open: boolean;
...
export function NspadelCreateGameConfirmModal({
  open,
  onOpenChange,
  club,
  bookings,
  ...
  onSuccess,
  flowMode = 'create',
}: NspadelCreateGameConfirmModalProps) {
  ...
  const runConfirm = useCallback(async () => {
    ...
    try {
      // Bookings are made under the player's Bandeja profile contact details;
      // the backend rejects with nspadelProfileContactRequired when missing.
      const provider = await createHydratedClubBookingProvider(club, {
        durationMinutes: bookings[0]?.durationMinutes,
      });
      if (!provider) throw new Error(t('club.booktime.errors.generic'));
      ...
      const snapshotInputs = buildBookingSnapshots(
        bookedIds.map((id, index) => {
          ...
          const dateKey = formatClubDateKey(entry.date, club);
          const endTime = buildNspadelEndTime(entry.startTime, entry.durationMinutes);
          return {
            uuid: id,
  ...
  return (
    <Dialog open={open} onClose={() => onOpenChange(false)} modalId={`nspadel-create-confirm-${club.id}`}>
          ...
          <ul className="space-y-2">
            {bookings.map((entry) => (
              <li key={entry.court.id} className="text-sm">
                ...
                />
                {' · '}
                {entry.startTime}–{buildNspadelEndTime(entry.startTime, entry.durationMinutes)}
              </li>
            ))}
          </ul>
