...
import { buildKlikterenEndTime } from '@/integrations/klikteren/slots';
...
type KlikterenCreateGameConfirmModalProps = {
  open: boolean;
...
export function KlikterenCreateGameConfirmModal({
  open,
  onOpenChange,
  club,
  klikterenVenueId: _klikterenVenueId,
  bookings,
  email,
  firstName,
  lastName,
  ...
  onSuccess,
  flowMode = 'create',
}: KlikterenCreateGameConfirmModalProps) {
  ...
  const runConfirm = useCallback(async () => {
    ...
    try {
      ...
      if (!provider) throw new Error(t('club.klikteren.errors.generic'));
      ...
      const snapshotInputs = buildBookingSnapshots(
        bookedIds.map((id, index) => {
          ...
          const dateKey = formatClubDateKey(entry.date, club);
          const endTime = buildKlikterenEndTime(entry.startTime, entry.durationMinutes);
          return {
            uuid: id,
  ...
  return (
    <Dialog open={open} onClose={() => onOpenChange(false)} modalId={`klikteren-create-confirm-${club.id}`}>
        ...
        <div className="space-y-4 px-6 pb-6">
          {reservedAsIdentity ? (
            <p className="text-sm text-gray-600 dark:text-gray-400">
              {t('club.klikteren.connectedAs', { email: reservedAsIdentity })}
            </p>
          ) : null}
          <ul className="space-y-2">
            {bookings.map((entry) => (
              <li key={entry.court.id} className="text-sm">
                ...
                />
                {' · '}
                {entry.startTime}–{buildKlikterenEndTime(entry.startTime, entry.durationMinutes)}
              </li>
            ))}
          </ul>
