...
export const PriceSection = ({
  ...
  const { t } = useTranslation();
  const effectiveCurrency = displayCurrency(priceProps.priceCurrency, priceProps.defaultCurrency);

  if (embedded) {
    return <PriceFields {...priceProps} showLabel />;
  }

  return (
    <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-4">
      <div className="flex items-center gap-2 mb-3">
        <span className="text-sm font-semibold text-gray-500 dark:text-gray-400">{getCurrencySymbol(effectiveCurrency)}</span>
        <h2 className="section-title">
          {t('createGame.price')}
        </h2>
      </div>
      <PriceFields {...priceProps} />
    </div>
  );
};
