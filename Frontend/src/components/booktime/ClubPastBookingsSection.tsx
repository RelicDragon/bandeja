...
import { useAllPastClubBookings } from '@/hooks/useAllPastClubBookings';
...
export function ClubPastBookingsSection({
  clubs,
  displaySettings,
  refreshKey = 0,
  showClubName = false,
  ...
  const clubById = useMemo(
    () => new Map(connectedClubs.map((c) => [c.clubId, connectedClubRowToBookingListClub(c)])),
    [connectedClubs],
  );
