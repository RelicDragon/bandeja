...
const FILTER_ID_CSS: Record<string, string> = {
...
function mediaAdjustFilterParts(adjust: StoryMediaAdjust): string[] {
  ...
  if (adjust.filterId && FILTER_ID_CSS[adjust.filterId]) {
    parts.push(FILTER_ID_CSS[adjust.filterId]);
  }
  return parts;
}

export function mediaAdjustToCssFilter(adjust: StoryMediaAdjust): string {
  const parts = mediaAdjustFilterParts(adjust);
  return parts.length ? parts.join(' ') : 'none';
}

/** Canvas 2D filter string — mirrors editor CSS preview for WYSIWYG export. */
