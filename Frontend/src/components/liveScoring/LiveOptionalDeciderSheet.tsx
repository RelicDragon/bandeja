...
import type { LiveAutomaticRecordMode, LiveOptionalDeciderFormat } from '@/utils/liveScoring';

type Props = {
  open: boolean;
  onChoose: (format: LiveOptionalDeciderFormat) => void;
  recordMode?: LiveAutomaticRecordMode | null;
  disabled?: boolean;
};

export function LiveOptionalDeciderSheet({ open, onChoose, recordMode, disabled = false }: Props) {
  const { t } = useTranslation();
  if (!open) return null;
  const regularCta =
    recordMode === 'AMERICANO_POINTS'
      ? t('gameDetails.liveScoring.deciderAmericanoCta')
