...
test.describe('authenticated navigation', () => {
  test('home shows bottom nav for inactive tabs', async ({ page }) => {
  ...
  test('can open Find tab', async ({ page }) => {
    await new ShellPage(page).gotoTab('/find');
  });

  test('can navigate to Chats tab', async ({ page }) => {
    await new ShellPage(page).expectAuthenticatedHome();
    await page.getByRole('button', { name: /^chats$/i }).click();
    await expect(page).toHaveURL(/\/chats/);
  });

  test('can navigate to Marketplace tab', async ({ page }) => {
    await new ShellPage(page).expectAuthenticatedHome();
